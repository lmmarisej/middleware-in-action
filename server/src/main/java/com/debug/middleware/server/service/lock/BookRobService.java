...
public class BookRobService {
    ...
    public void robWithZKLock(BookRobDto dto) throws Exception {
        ...
        try {
            if (mutex.acquire(15L, TimeUnit.SECONDS)) { //采用互斥锁组件尝试获取分布式锁-其中尝试的最大时间在这里设置为15s 当然,具体的情况需要根据实际的业务而定
                //根据书籍编号查询记录
                BookStock stock = bookStockMapper.selectByBookNo(dto.getBookNo());
                //统计每个用户每本书的抢购数量
                int total = bookRobMapper.countByBookNoUserId(dto.getUserId(), dto.getBookNo());
                ...
                if (stock != null && stock.getStock() > 0 && total <= 0) {
                    ...
                    int res = bookStockMapper.updateStock(dto.getBookNo());     //当前用户抢购到书籍，库存减一
                    if (res > 0) {      //更新库存成功后，需要添加抢购记录
                        BookRob entity = new BookRob();
                    ...
                    throw new Exception("该书籍库存不足!");        //如果不满足上述的任意一个if条件，则抛出异常
                }
            } else {
    ...
    public void robWithRedisson(BookRobDto dto) throws Exception {
        ...
        try {
            boolean result = lock.tryLock(100, 10, TimeUnit.SECONDS);   // 尝试加锁最多等待100秒，10秒后自动解锁
            if (result) {
                BookStock stock = bookStockMapper.selectByBookNo(dto.getBookNo());  //根据书籍编号查询记录
                int total = bookRobMapper.countByBookNoUserId(dto.getUserId(), dto.getBookNo());    //统计每个用户每本书的抢购数量
                //商品记录存在、库存充足，而且用户还没抢购过本书，则代表当前用户可以抢购
                if (stock != null && stock.getStock() > 0 && total <= 0) {
                    int res = bookStockMapper.updateStockWithLock(dto.getBookNo());     //当前用户抢购到书籍，库存减一
                    //int res=bookStockMapper.updateStock(dto.getBookNo());     //如果允许商品超卖-达成饥饿营销的目的，则可以调用下面的方法
                    if (res > 0) {      //更新库存成功后，需要添加抢购记录
                        BookRob entity = new BookRob();
                        entity.setBookNo(dto.getBookNo());
                        entity.setUserId(dto.getUserId());
